...
# 영상 프레임 내의 화재 연기 색상 값 추출하여 화재 판단
def detect_smoke(frame):
    ...
    # 그레이 스케일 이미지 이진화
    _, thresholded_frame = cv2.threshold(gray_frame, lower_bound, upper_bound, cv2.THRESH_BINARY)

    # 연기가 존재하는 부분의 픽셀 개수 계산
    smoke_pixel_count = np.count_nonzero(thresholded_frame)

    # 임계값 설정
    threshold = 10000

    if smoke_pixel_count > threshold:
        return True
    else:
        return False
...
def process_detection(detections):
    ...
    for (_, _, _, _), tracker_id in zip(detections.xyxy, detections.tracker_id):
        ...
        # 객체 사라짐
            ...
            # 여기서 duration을 사용하여 필요한 처리 수행 -> 사용자 인풋 값으로 설정(?)
...
def process_faces(frame, detections):
    global faces_info
...
def process_frame(frame, frame_index):
    ...
    # 얼굴 인식 부분 추가
    img_with_faces = process_faces(frame, detections)
    ...
    while True:
        ...
        if rtsp_capture.get_frame_count() % 20 == 0:
            ...
            if frame is not None:
                frame_index = rtsp_capture.get_frame_count()
